...
export default function NavItem({ item, level, isParents = false, setSelectedID }) {
  ...
  const itemIcon = item?.icon ? (
    <Icon stroke={1.5} size={drawerOpen ? '20px' : '21px'} style={{ ...(isParents && { fontSize: 20, stroke: '1.5' }) }} />
  ) : (
    ...
    <>
      <ListItemButton
        ...
        sx={{
          ...
          mb: 0.5,
          ...(drawerOpen && level !== 1 && { ml: `${level * 12}px` }),
          ...(!drawerOpen && { pl: 1.25 }),
          ...(drawerOpen &&
            level === 1 && {
            '&:hover': {
        ...
        <ButtonBase aria-label="theme-icon" sx={{ borderRadius: `${borderRadius}px` }} disableRipple={drawerOpen}>
          <ListItemIcon
            sx={{
              ...
              color: isSelected ? iconSelectedColor : 'grey.500',
              ...(!drawerOpen &&
                level === 1 && {
                borderRadius: `${borderRadius}px`,
                width: 46,
